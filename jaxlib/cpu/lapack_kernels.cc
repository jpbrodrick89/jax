...
ffi::Error OrthogonalQrMultiply<dtype>::Kernel(
  ...
  FFI_ASSIGN_OR_RETURN(auto c_rows_v,
                       MaybeCastNoOverflow<lapack_int>(c_rows));
  FFI_ASSIGN_OR_RETURN(auto c_cols_v,
  ...
  for (int64_t i = 0; i < batch_count; ++i) {
    fn(&side_v, &trans_v, &c_rows_v, &c_cols_v, &k_v, a_data, &lda_v,
       tau_data, c_out_data, &c_leading_dim_v, work_data.get(), &work_size_v,
       &info);
...
template <ffi::DataType dtype>
int64_t OrthogonalQrMultiply<dtype>::GetWorkspaceSize(
    char side, char trans, lapack_int m, lapack_int n, lapack_int k,
    lapack_int lda) {
  ValueType optimal_size = {};
  lapack_int c_leading_dim = m;
  lapack_int info = 0;
  lapack_int workspace_query = -1;
  fn(&side, &trans, &m, &n, &k, nullptr, &lda, nullptr, nullptr,
     &c_leading_dim, &optimal_size, &workspace_query, &info);
  return info == 0 ? static_cast<int64_t>(std::real(optimal_size)) : -1;
}
